              ...
              <van-cell center border v-for="log in item.logs" :key="log.id">
                <template #icon>
                  <div class="text-center pl-2 pr-5 border-r w-20">
                    <div class="text-xs text-gray-500">{{ formatDate(log.date, 'YYYY') }}</div>
                    <div class="text-gray-700 mb-1">{{ formatDate(log.date, 'MMMM') }}</div>
                    <div class="font-bold text-lg">{{ formatDate(log.date, 'D') }}</div>
                  </div>
                </template>
